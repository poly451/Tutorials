...
def get_dict(filepath):
    ...
    mydict = {}
    for elem in mylines:
        mydict = key_value_pair(elem, mydict)
    return mydict

def get_npc_file(npc_name, npc_kind):
    char_name, char_kind = get_char_info()
    char_dir = "{}_{}".format(char_name, char_kind)
    npc_filename = "{}_{}.txt".format(npc_name, npc_kind)
    mypath = os.path.join("data", "player_files", char_dir, "NPCs", npc_filename)
    if p: print("utils.get_npc_file: {}".format(mypath))
    # ----
...
def string_to_list(mystring):
    divider = ";"
    temp_list = mystring.split(divider)
    temp_list = [int(i.strip()) for i in temp_list if len(i.strip()) > 0]
    return temp_list
...
if __name__ == "__main__":
    mystring = "Monkeys love eating ripe mangos. But lately the fruit hasn't had a chance to become ripe because a fungus has started growing on the trees, causing the fruit to fall off the tree while it is still ripe. Kinto mushrooms used to grow by the mango trees. They kept the fungus in check. But the mushrooms have been eaten by a new species of bird that came into the area because they haven't been able to find nesting materials. They came here, saaw the mushrooms and started snacking. If these birds were provided with nesting materials then they would go back to where they came from. The mushrooms would regrow, the mangos would ripen and the monkeys would be too busy eating to attack the Inn's customers. So the Inn would be profitable and wouldn't have to lay anyone off."
    s = limit_line_length(mystring, line_length=30)
    print(s)
    # mystring = "3;4"
    # this_list = string_to_list(mystring)
    # [print(i) for i in this_list]
    # marty_dict = get_npc_file("marty", "provisioner")
    # print(marty_dict)
