...
class Player:
    ...
    def move(self, dx, dy, obstacles=None):
        if obstacles is None:
            ...
            print("def player.move, above test: x,y: {}, {}".format(self.x, self.y))
            col_result = pygame.sprite.spritecollideany(self.player_body, obstacles.inner, collided=None)
            # ----
            if col_result is not None:
                # There was a collision.
                # self.movement_okay = True
                print("Player encountered obstacle and therefore will not move: {}".format(col_result))
                self.y = round(previous_y, 0)
                self.x = round(previous_x, 0)
                self.rect = self.player_body.image.get_rect()
                self.rect = self.player_body.rect.move(self.x * constants.TILESIZE, self.y * constants.TILESIZE)
                print("def player.move, below test: x,y: {}, {}".format(self.x, self.y))
                return False
            else:
                return True
