...
class MainDialog:
    def __init__(self, height=400, line_width=50):
        ...
        self.window_text_list = []
        self.assemble_menu()
        self.menu_activa = "main_menu"  # sub_menu

    def assemble_menu(self):
        filepath = os.path.join("data", "quizes")
        menu_items = utils.get_directories(filepath)
        self.window_text_list = ["{}. {}".format(count+1, i) for count, i in enumerate(menu_items)]
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    self.text = ""
                    if self.menu_activa == "main_menu":
                        self.process_directory_input()
                        self.menu_activa = "sub_menu"
                    elif self.menu_activa == "sub_menu":
                        self.process_sub_directory_input()
                else:
                    self.text += event.unicode

    def process_sub_directory_input(self):
        ...
        if not myint in list(range(1, len(self.window_text_list)+1)):
            return False
        # -----------------------------------------------------
        print("In process_sub_directory_input in dialog02.py")
        dir = self.window_text_list[myint-1]
        myint = dir.find(" ")
        self.message = dir[myint:].strip()
        print(self.message)
        self.keep_looping = False
    ...
    def process_directory_input(self):
        def is_valid(mystring):
            ...
            if mystring.find("_data_") > -1: return False
            if mystring.find("_scores_") > -1: return False
            return True
        # -----------------------------------------------------
        if not utils.is_int(self.user_text): return False
        myint = int(self.user_text)
        ...
        self.directory = dir
        print(dir)
        filepath = os.path.join("data", "quizes", dir)
    ...
    def main(self):
        ...
        while self.keep_looping:
            ...
            self.update()
            self.draw()
        return os.path.join("data", "quizes", self.directory, self.message)
