...
if __name__ == "__main__":
    ...
    with open(filepath, "r") as f:
        mynames = f.readlines()
        mynames = [i.strip() for i in mynames if len(i.strip()) > 0]
    for name in mynames:
        print(name)
        name = swap_first_vowel_and_last_vowel(name)
        name = put_last_letter_second(name)
        print(name)
    # new_name = swap_vowels("karen")
    # names = get_random_NPC_name(race="human", sex="male", number_of_names=10)
